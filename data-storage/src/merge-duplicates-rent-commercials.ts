...
import { RentCommercialsModel } from './schemas';
...
async function mergeDuplicatesRentCommercials() {
  await RentCommercialsModel.aggregate([
    {
      $sort: { _id: 1 },
    },
    {
...
try {
  mongoose.connect(DSN)
    .then(mergeDuplicatesRentCommercials)
    .then(() => process.exit(0));
} catch (err) {
  debugError(err);
}
